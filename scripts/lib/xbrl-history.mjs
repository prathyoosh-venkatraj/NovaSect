...
// Concept synonym map (mirrors generate-report.mjs CONCEPTS). Exported so the
// live sec-proxy extracts the same series the offline pipeline uses.
//
// Each list is US-GAAP tags FIRST, then the IFRS (ifrs-full) equivalents
// appended — so 20-F filers that report under IFRS (e.g. SHEL, BP, TSM, RELIANCE)
// populate instead of degrading to N/A. extractAnnual() searches the us-gaap,
// ifrs-full and dei namespaces in order, so a US filer still resolves to its
// GAAP tag and an IFRS filer (no us-gaap facts) falls through to the IFRS tag.
// Tags shared verbatim across both taxonomies (Assets, Liabilities) need no
// IFRS-specific entry — the namespace fallthrough handles them.
...
export function historyData(x) {
  ...
  return {
    ...
    summary: {
      ...
      totalAssets:       pick('ta'),
      cash:              pick('cash'),
      interestExpense:   pick('ie'),
      dividendsPaid:     pick('div'),
      sharesOutstanding: pick('sharesOut'),
    },
    ratios: {
